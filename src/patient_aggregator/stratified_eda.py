...
from .config_loader import get_severity_prescription_order
...
def get_stratification_column(df: pd.DataFrame, config: Dict[str, Any], 
    ...
    When multiple values exist, the prescribed severity is the first match in
    severity_prescription_order (worst severity wins). Applies group ordering
    from config and filters excluded groups.
    ...
    Returns:
        Series with single value per patient (prescribed by severity order if multiple)
    """
    ...
    prescription_order = get_severity_prescription_order(config)
    ...
    def get_primary_value(values_str):
        ...
        if len(values) == 1:
            return str(values[0])
        # Prescribed severity = first in prescription order (worst severity wins)
        str_values = [str(v) for v in values]
        for severity in prescription_order:
            if severity in str_values:
                return severity
        # Fallback: value not in prescription list (e.g. typo); use first in list
        return str_values[0]
...
def create_stratified_distributions(df: pd.DataFrame, stratification_col: pd.Series,
    ...
    try:
        if len(stratification_col) == 0:
            tqdm.write("  ⚠ No stratification data available for distribution plots")
            return
        ...
        if not available_features:
            tqdm.write(f"  ⚠ No available features for distribution plots (requested: {len(features)}, available: 0)")
            return
        ...
        tqdm.write(f"  ⚠ Error creating distribution plots: {e}")
        import traceback
...
def create_stratified_boxplots(df: pd.DataFrame, stratification_col: pd.Series,
    ...
    try:
        if len(stratification_col) == 0:
            tqdm.write("  ⚠ No stratification data available for box plots")
            return
        ...
        if len(groups) == 0:
            tqdm.write("  ⚠ No groups available for box plots")
            return
        ...
        available_features = [f for f in features if f in df_strat.columns]
        if not available_features:
            tqdm.write(f"  ⚠ No available features for box plots (requested: {len(features)}, available: 0)")
            return
        ...
        tqdm.write(f"  ⚠ Error creating box plots: {e}")
        import traceback
        tqdm.write(traceback.format_exc())
...
def create_feature_comparison_plots(df: pd.DataFrame, stratification_col: pd.Series,
    ...
    try:
        if len(stratification_col) == 0:
            tqdm.write("  ⚠ No stratification data available for feature comparison plots")
            return
        ...
        if not derived_features:
            tqdm.write(f"  ⚠ No derived features available for comparison plots (requested: {len(features)}, available: 0)")
            return
        ...
        tqdm.write(f"  ⚠ Error creating feature comparison plots: {e}")
        import traceback
        tqdm.write(traceback.format_exc())
...
def create_stratified_plots(df: pd.DataFrame, config: Dict[str, Any], output_dir: Path):
    ...
    available_plot_features = [f for f in plot_features if f in df_filtered.columns]
    missing_plot_features = [f for f in plot_features if f not in df_filtered.columns]
    
    if available_plot_features:
        tqdm.write(f"  ✓ Available plot features ({len(available_plot_features)}/{len(plot_features)}): {', '.join(available_plot_features[:5])}{'...' if len(available_plot_features) > 5 else ''}")
    if missing_plot_features:
        tqdm.write(f"  ⚠ Missing plot features ({len(missing_plot_features)}): {', '.join(missing_plot_features[:5])}{'...' if len(missing_plot_features) > 5 else ''}")
    
    available_stat_features = [f for f in features_for_statistics if f in df_filtered.columns]
    missing_stat_features = [f for f in features_for_statistics if f not in df_filtered.columns]
    
    if features_for_statistics:
        if available_stat_features:
            tqdm.write(f"  ✓ Available statistics features ({len(available_stat_features)}/{len(features_for_statistics)}): {', '.join(available_stat_features[:5])}{'...' if len(available_stat_features) > 5 else ''}")
        if missing_stat_features:
            tqdm.write(f"  ⚠ Missing statistics features ({len(missing_stat_features)}): {', '.join(missing_stat_features[:5])}{'...' if len(missing_stat_features) > 5 else ''}")
    
    tqdm.write("")  # Blank line for readability
    
    # Create simplified plots - only essential visualizations
    ...
    try:
        ...
        tqdm.write(f"  ⚠ Failed to create stratified distributions: {e}")
    ...
    try:
        ...
        tqdm.write(f"  ⚠ Failed to create stratified box plots: {e}")
    
    # Generate simplified summary statistics table
    if config.get('generate_summary_tables', True):
        tqdm.write("Generating summary statistics table...")
